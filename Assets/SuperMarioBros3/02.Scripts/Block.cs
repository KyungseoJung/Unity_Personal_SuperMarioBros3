...
// #35
        ...
        switch(blockType)           // #25 조건 추가
        {
            ...
            case BLOCK_TYPE.PBUTTON:        // #26 PBUTTON 블록을 플레이어가 머리로 쳤을 때
                ...
                {   
                    ...
                    Debug.Log("//#4 보완 : 플레이어의 X 위치" + other.gameObject.transform.root.position.x);
            ...
            case BLOCK_TYPE.ITEMBLOCK_GREEN:    // #60
                ...
                if(other.gameObject.tag == "ShellWeapon" && !isTouched)       // #22 보완 - 한번만 등장하도록 isTouched 변수 추가
                {
                    ...
                    if(other.gameObject.GetComponent<EnemyCtrl>().enemyType == EnemyCtrl.ENEMY_TYPE.SHELL)  // #22 거북 등껍질에 부딪혔을 때에도 블록 부숴지도록
                    {
                        ...
                        // Debug.Log("//#4 보완 : 껍질의 X 위치" + other.gameObject.transform.root.position.x);
    ...
    void BlockIsTouched(int _itemDir = 1)    // #2 플레이어 headCheck와 블록 부딪힌 후 작동
    {
        switch(blockType)   // 블록 타입에 따라 다르게 작동
        {
            ...
            case BLOCK_TYPE.ITEMBLOCK : 
            {
                ...
                switch(playerLife.playerLevel)  // #4 #5 플레이어 레벨에 따라 다른 아이템 등장함
                {
                    ...
                    case PlayerLife.MODE_TYPE.LEVEL1 : 
                        StartCoroutine(ItemAppears(Item.ITEM_TYPE.MUSHROOM, _itemDir));   // #2 보완 - 코루틴 활용 - 코루틴은 항상 생성 or 소멸에만 사용하라고 배운 기억    // #4 보완
                        break;
                    case PlayerLife.MODE_TYPE.LEVEL2 : 
                        StartCoroutine(ItemAppears(Item.ITEM_TYPE.LEAF, _itemDir));
                        break;
            ...
            {                
                ...
                AudioSource.PlayClipAtPoint(blockClips[1], transform.position);
                StartCoroutine(ItemAppears(Item.ITEM_TYPE.GREENMUSHROOM, _itemDir));   // 버섯 등장
                // 목숨 1 증가
    ...
    IEnumerator ItemAppears(Item.ITEM_TYPE _type, int _itemDir = 1)  // #4 #5 아이템(버섯 or 나뭇잎) 등장
    {
        ...
        switch(_type)
        {
            case Item.ITEM_TYPE.MUSHROOM : 
                ...
                Debug.Log("//#4 보완 : Block 스크립트// 버섯 이동 방향은 " + _itemDir);
            ...
            case Item.ITEM_TYPE.GREENMUSHROOM : // #60
                ...
                Debug.Log("//#4 보완 : Block 스크립트// 초록 버섯 이동 방향은 " + _itemDir);
                GameObject greenMushroom = Instantiate(greenMushroomObj, transform.position, Quaternion.identity); // 생성 (이때 생성된 버섯 or 나뭇잎은 isTrigger 체크되어 있어야 함. 블록과 충돌처리 되지 않도록)
                if(_itemDir == -1)
                    greenMushroom.GetComponent<Item>().Flip();   // #4 블록에 부딪힌 플레이어 위치에 따라 버섯 이동 방향 설정 
